...
smisc::ipak(c("utils", "tidyr", "dplyr", "ggplot2", "DescTools",
              "e1071", "pracma", "gridExtra", "data.table", "tables", "zoo",
              "tidyverse", "parallel", "scales"))

# Main function begins (encompasses other functions) — up 90 seconds to run, on Windows can be > 3 min
data_graphing <- function(catch_trial_cutoff,  block_acc_cutoff, catch,
                          ...
                          sep_vis_fields, smooth_method,  dep_var,
  ...
  ptcpt_tabulate <- function(participant_num){
    ...
    fread(ptcpt_path, select = c(1:23)) %>%                                     # Reads in participant data (the 'select' part is because PsychoPy created an empty column at the end of the data frame for the first few participants, which meant those dataframes had different dimensions than subsequent data frames, which 'do.call' doesn't like)
      ...
      filter(
             between(Acc_blocks_unfiltered, lowacc_ptcpts_pre_block_filter,     # Filters out participants whose non-catch, pre-block-filtering accuracy is outside of desired range
                     highacc_ptcpts_pre_block_filter),     
      ...
      mutate(Acc = ifelse(((Opacity > 0 &                                       # Overwrites 'Acc' column, since original PsychoPy code was written to count any trial with
             ...
             CTI = RoundTo(lilsquareStartTime - flash_circleEndTime,
                                   sampling_freq),
             RT = ifelse(Acc == 1 | ButtonPressTime - lilsquareStartTime > .2,  #                indicating RT after target appeared on screen, only for correct trials with an RT < 200 ms 
                         ButtonPressTime - lilsquareStartTime, NA),   
             Stim_Sides = as.character(                                         
               ifelse(CorrSide == FlashSide, "Congruent", "Incongruenet")),     #                indicating whether cue was congruent or incongruent
      ...
      mutate(Trials_below_block_cutoff = sum(is.na(Acc))/n(),                   # Creates column indicating proportion of all trials in a block below 'block_acc_cutoff'
             Acc_blocks_filtered = mean(Acc, na.rm = TRUE)) %>%                 #                indicating mean accuracy for non-catch trials; note this is after before we've filtered for block_acc_cutoff, unlike Acc_blocks_unfiltered
  ...
  time_course_graph_facets <- rbind((ptcpts_tabulated %>%                       # Copies each row in 'ptcpts_tabulated' except 'participant' = 'All', which creates the top graph on the left labeled 'All'
    ...
    ggplot(aes(CTI, !!dep_var, group = Stim_Sides, color = Stim_Sides)) +       # CTI is x axis, dep_var is y axis
      geom_line(alpha = I(2/10), color = "grey", show.legend = FALSE) +         # Unsmoothed data is graphed lightly in gray
  ...
  hann <- hanning.window(length(wins))                                          # Create Hanning window, gets applied later
  
  
  # Function interpolates and returns amplitude for each participant (assuming they haven't already been filtered out above)
  amplitude_func <- function(participant_num){
   ...
   Sides <- levels(factor(exp_trials_noncatch$Stim_Sides))
    ...
    # Function interpolates as a sliding window
    ...
    as.data.frame(t(mcmapply(slidingwindow, wins))) %>%
      modify(as.numeric) %>%
  	  mutate_at(vars(!!Sides), funs(Mod(fft(detrend(.)*hann)))) %>%             # Detrend, multiply by Hanning window, apply FFT, and then take magnitude
      modify(as.numeric) %>%
      mutate(Hz = (row_number()-1)/(n()*win_freq))
  }
  
  # Call 'amplitude_func' for all remaining participants, then combine the data into one data frame; the interpolating is what makes this line slow the whole script
  ...
  grid.arrange(time_course_graph_facets, fft_facets, ncol = 2)                  # Combines time series and FFT graphs into one plot
  
  # Saves data frame to 'analysis' folder if 'save_output' argument == 'Yes'
  if (save_output == "Yes"){
      side_by_side <- arrangeGrob(time_course_graph_facets, fft_facets,
                                  ncol = 2)
      ggsave(file.path("analysis", paste0("Time-Series_+_FFT_Plots_", dep_var,
                                          "_low_", lowacc_ptcpts, "_high_",
                                          highacc_ptcpts, ".pdf")),
...
data_graphing(catch_trial_cutoff = .85,                                         # Filters out participants with a catch trial accuracy below this value
              ...
              lowacc_ptcpts_pre_block_filter = .1,                              # Filters participants by their accuracy before their blocks below 'block_acc_cutoff' have been interpolated over 
              highacc_ptcpts_pre_block_filter = 1,                              # Line above is the floor of the filter, this line is the ceiling
              lowacc_ptcpts_post_block_filter = .4,                             # Filters participants by their accuracy after their blocks below 'block_acc_cutoff' have been interpolated over 
              ...
              ptcpts = 201:230,
              save_output = "Yes")                                              # Use "Yes" and "No" (if 'no', output will still be visible in R)
