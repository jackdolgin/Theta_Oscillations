...
pacman::p_load(utils, tidyr, dplyr, ggplot2, DescTools, bspec, pracma,
               gridExtra, data.table, tables, zoo, scales, stats, viridis,
               gginnards, purrr, stingr, rlang)
...
main_function <- function(display, dset, wm_exp, iso_sides, sbtr, samp_per,
  ...
  amplitude <- function(x, shuf){
    ...
    cmbd %>%     
      ...
      add_row(
        ...
        !!dvcol := 0) %>%
      group_by(participant, Stim_Sides) %>%
      mutate(Power = fft(!!dvcol) %>% `*`(sqrt(2 / n())) %>%
               Mod %>% `^` (2)) %>%
  ...
  fillerfunc <- function(Hz0, Stim_Sides0, Power0, xray0, yray0){
  ...
  map_dfr(1, ~amplitude(.x, FALSE)) %>%
    ungroup() %>%
    mutate(
      ntile = pmap_dbl(
        list(Hz0 = Hz, Stim_Sides0 = Stim_Sides,Power0 = Power, xray0 = "ntile",
             yray0 = "Power0 > Power | Power == min(Power)"), fillerfunc),
      cutoff = pmap_dbl(
        list(Hz0 = Hz, Stim_Sides0 = Stim_Sides, Power0 = Power,
             xray0 = "Power", yray0 = "ntile < 1 - α"), fillerfunc)) %>%
    group_by(Stim_Sides) %>%
    mutate_at(vars(cutoff), list(~p.adjust(., method = mult_correcs))) 
  
  
    mutate(ntile = pmap_dbl(., function(Hz, Stim_Sides, Power, ...){           # For each row in `amps`...
      filler <- tasco %>%                                                        # ... finds row in `amps_shuff` that...
        rename(Hz0 = Hz, Stim_Sides0 = Stim_Sides, Power0 = Power) %>%
        filter(Hz0 == Hz,                                                     # ... 1) matches in the Hz column...
               Stim_Sides0 == Stim_Sides)                                         # ... 2) matches sides locations (e.g. `Invalid`)...
      filler %>%
        filter( Power > Power0 | Power0 == min(Power0)) %>%                                            # ....3) with a smaller Power0 than `amps`' Power...
        top_n(1, Power0) %>%                                                   # ... and 4) with largest Power0 among the remaining rows (we are being conservative and ntile reference...
        pull(ntile)                                                            # .... Power0 is always a smaller power than would be Power)
    })) %>%
    group_by(Stim_Sides) %>%
  ...
  tata1 <- map_dfr(1, ~amplitude(.x, FALSE)) %>%
    ungroup() %>%
    mutate(ntile = pmap_dbl(list(Hz0 = Hz, Stim_Sides0 = Stim_Sides, Power0 = Power, xray0 = "ntile", yray0 = "Power0 > Power | Power == min(Power)"), fillerfunc))
  
  tata1 <- map_dfr(1, ~amplitude(.x, FALSE)) %>%
    ungroup() %>%
    mutate(ntile = pmap_dbl(list(Hz0 = Hz, Stim_Sides0 = Stim_Sides, Power0 = Power, xray0 = "Power", yray0 = "ntile < 1 - α"), fillerfunc))
  ...
  tata <- map_dfr(1, ~amplitude(.x, FALSE)) %>%
    ungroup() %>%
    mutate(ntile = pmap_dbl(., function(Hz, Stim_Sides, Power, ...){           # For each row in `amps`...
      filler <- tasco %>%                                                        # ... finds row in `amps_shuff` that...
        rename(Hz0 = Hz, Stim_Sides0 = Stim_Sides, Power0 = Power) %>%
...
### 1 works
  ...
  fillerfunc <- function(Hz0, Stim_Sides0, Power0, xray0){
    tasco %>%                                                        # ... finds row in `amps_shuff` that...
      filter(Hz0 == Hz, Stim_Sides0 == Stim_Sides)  %>%
      filter( Power0 > Power | Power == min(Power)) %>%                                            # ....3) with a smaller Power0 than `amps`' Power...
      top_n(1, Power) %>%                                                   # ... and 4) with largest Power0 among the remaining rows (we are being conservative and ntile reference...
      pull(ntile)  
  }
  ...
  tata1 <- map_dfr(1, ~amplitude(.x, FALSE)) %>%
    ungroup() %>%
    mutate(ntile = pmap_dbl(list(Hz0 = Hz, Stim_Sides0 = Stim_Sides, Power0 = Power, xray0 = Stim_Sides), fillerfunc))
...
##### take 2
  ...
  fillerfunc <- function(Hz0, Stim_Sides0, Power0, xray0, yray0){
    ...
    tasco %>%                                                        # ... finds row in `amps_shuff` that...
      filter(Hz0 == Hz, Stim_Sides0 == Stim_Sides)  %>%
      filter( Power0 > Power | Power == min(Power)) %>%                                            # ....3) with a smaller Power0 than `amps`' Power...
      top_n(1, !!yray0) %>%                                                   # ... and 4) with largest Power0 among the remaining rows (we are being conservative and ntile reference...
      pull(xray0)  
  }
  ...
  tata <- map_dfr(1, ~amplitude(.x, FALSE)) %>%
    ...
    mutate(ntile = pmap_dbl(., function(Hz, Stim_Sides, Power, ...){           # For each row in `amps`...
      tasco %>%                                                          # ... finds row in `amps_shuff` that...
        ...
        filter(Hz0 == Hz,                                                     # ... 1) matches in the Hz column...
               Stim_Sides0 == Stim_Sides,                                         # ... 2) matches sides locations (e.g. `Invalid`)...
               Power > Power0 | Power0 == min(Power0)) %>%                                            # ....3) with a smaller Power0 than `amps`' Power...
        top_n(1, Power0) %>%                                                   # ... and 4) with largest Power0 among the remaining rows (we are being conservative and ntile reference...
        pull(ntile)                                                            # .... Power0 is always a smaller power than would be Power)
    })) %>%
    group_by(Stim_Sides) %>%
    mutate_at(vars(ntile), list(~p.adjust(., method = mult_correcs))) %>%
    mutate(fillfornow = pmap_dbl(. function(Hz, Stim_)))
    
  
    # mutate_at(vars(ntile), list(~p.adjust(., method = mult_correcs) / .)) %>%
    select(Hz, Stim_Sides, ntile) %>%
    safe_right_join(tasco, by = c("Hz", "Stim_Sides"),
                    conflict = `*`) %>%
    group_by(Hz, Stim_Sides) %>%
