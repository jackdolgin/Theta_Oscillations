...
pacman::p_load(
  ...
  zoo, scales, stats, viridis, gginnards, purrr, stingr, rlang, here)
...
main_function <- function(averaging, visualize, batch, wm_exp, iso_sides, sbtr, samp_per,
  ...
  t_srs_g <- function(x){
    cmbd %>%
      group_by(Stim_Sides, CTI) %>%
      summarise_at(vars(!!dvcol), list(~qnorm(.975) * std_err(.)))
  ...
  grouping_amps <- if (visualize == "Individual"){
  ...
  amplitude <- function(x, to_shuf){
    ...
    cmbd_summarize(to_shuf) %>%
      ...
      mutate(Power = fft(!!dvcol) %>%
               `*`(sqrt(2 / n())) %>%
               Mod %>% `^` (2)) %>%
      mutate(Hz = (rank(row_number()) - 1) / (n() * samp_per)) %>%
      filter(
        rank(Hz) - 1 <= floor(n_distinct(Hz) / 2),                  
        Hz < xmax) %>%
      group_by(Hz, Stim_Sides, !!!grouping_amps) %>%
...
main_function(averaging = "individual", #individual or grouped
