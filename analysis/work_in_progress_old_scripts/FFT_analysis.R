...
main_function <- function(display, dset, wm_exp, iso_sides, sbtr, samp_per,
  ...
  pcpts_combine <- function(pcpt){
    fread(file.path("data", pcpt, paste0(pcpt, ".csv"))) %>%                    # Reads in participant data
      ...
      ungroup() %>%
      mutate_at(vars(Acc, RT), list(~ifelse(                                    # Changes `Acc` and `RT` (dv) column values to NA if...
  ...
  cmbd <- map_dfr(pcpts, pcpts_combine) %>%                                     # Calls `pcpts_combine` function for argument `pcpts` + combines each participant's dataframe into one
    group_by(CTI, Stim_Sides, !!!grouping_cnsts, participant) %>%
    summarise_at(vars(Acc, RT), list(~mean(., na.rm = TRUE))) %>%               # Overwrites dv columns according to mean of each combination of `CTI`, `Stim_Sides`, and `participant`
    arrange(Acc_prefilter, participant, Stim_Sides, CTI) %>%                    # Sets the order for individuals' plots
