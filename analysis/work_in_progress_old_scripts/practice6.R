...
main_function <- function(display0, display, dset, wm_exp, iso_sides, sbtr, samp_per,
  ...
  dv_mutate <- function(x, s){ mutate_at(x, vars(!!dvcol), list(~!!s))}
  ...
  grouping_cnsts <-
    if(display0 == "individual"){
      quos(participant, Trials_filtered_out, Acc_prefilter, Acc_postfilter,       # Columns that are frequently used for grouping, variable means...
         CatchAcc)
      } else{quos()}
  
  cmbd_summarized <- function(to_shuf){
    cmbd %>%
  ...
  win <- if (win_func == "Tukey"){
    tukeywindow(length(CTIs), .5)} else {
      match.fun(paste0(tolower(win_func), "window")) (length(CTIs))}
  ...
  conf_int <- function(x, y){
      summarise_at(x, vars(y), list(~qnorm(.975) * std_err(.)))
  }
  
  amplitude <- function(x, to_shuf){
    set.seed(x)
    cmbd_summarized(to_shuf) %>%
  ...
  amped_up <- map_dfr(1, ~amplitude(.x, FALSE)) %>%
    ...
    mutate(cutoff = map_dbl(
      list(Hz0 = Hz, Stim_Sides0 = Stim_Sides, Power0 = Power, xray0 = "Power",
           yray0 = "ntile <  zray0 * (1 - α)", zray0 = ntile2), fillerfunc))
...
main_function(display0 = "individual", #individual or grouped
              
              display = "Time-Series Across Participants",                              # Either `FFT Across Participants`, `Time-Series Across Participants`, `Time-Series + FFT by...
