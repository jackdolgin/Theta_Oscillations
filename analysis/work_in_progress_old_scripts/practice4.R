...
main_function <- function(display, dset, wm_exp, iso_sides, sbtr, samp_per,
  ...
  pcpts_combine <- function(pcpt){
    fread(file.path(dset, pcpt, paste0(pcpt, ".csv"))) %>%                    # Reads in participant data
  ...
  amped_up <- map_dfr(1, ~amplitude(.x, FALSE)) %>%
    group_by(Stim_Sides) %>%
    mutate(
      ntile2 = pmap_dbl(list(
        Hz0 = Hz, Stim_Sides0 = Stim_Sides, Power0 = Power, xray0 = "ntile",
        yray0 = "Power0 > Power | Power == min(Power)"), fillerfunc),
      ntile2 = p.adjust(ntile2, method = mult_correcs) / ntile2,
      cutoff = pmap_dbl(list(
        Hz0 = Hz, Stim_Sides0 = Stim_Sides, Power0 = Power,  xray0 = "Power",
