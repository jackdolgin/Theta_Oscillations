...
pacman::p_load(
  ...
  zoo, scales, stats, viridis, gginnards, purrr, stingr, rlang, here, binom)
...
main_function <- function(averaging, visualize, batch, wm_exp, iso_sides, sbtr, samp_per,
  ...
  cmbd_summarize <- function(to_shuf){
    cmbd %>%
      ...
      summarise(
        Conf_Int = ifelse(
          !!to_shuf | averaging == "Individual", 0,
  ...
  time_srs_graph <- cmbd_summarize(FALSE) %>%
    ...
    stat_smooth(
      ...
      show.legend = ifelse(c("time_series", "FFT") %in% display, FALSE, TRUE))
  ...
  amped_up <- map_dfr(1, ~amplitude(.x, FALSE)) %>%
    ...
    mutate(`Significance Cutoff` = pmap_dbl(
  ...
  fft_aggregate_graph + 
    ...
    # filter(Observed > `Significance Cutoff`) %>%
    filter(Observed < `Significance Cutoff`) %>%
    select(-c(`Significance Cutoff`, ntile2, Conf_Int)) %>%
    # select(-c(`Significance Cutoff`, ntile2)) %>%
    pivot_longer(Observed, "Power", values_to = "source")
  ...
  fft_graph_1 <- ggplot(amped_up,
                        aes(Hz,
                            Observed,
                            col = Stim_Sides,
                            linetype = line_col,
                            fill = Stim_Sides,
                            ymin = Observed - Conf_Int,
                            ymax = Observed + Conf_Int))
  ...
  # amped_up_prelim_graph <- amped_up %>%
  ...
  time_srs_graph_pt_1 +
    ...
    # stat_smooth(method = tolower(smooth_method), span = 0.2, se = FALSE,      
